                    ...
                    LazyVStack(spacing: 1) {
                        ForEach(documentState.document.layers.reversed()) { layer in
                            LayerRow(
                                ...
                                onSelect: { documentState.selectedLayerID = layer.id },
                                onToggleVisibility: { documentState.toggleLayerVisibility(id: layer.id) },
                                onToggleLock: { documentState.toggleLayerLock(id: layer.id) }
                            )
                        }
                    }
                    .padding(.vertical, 8)
